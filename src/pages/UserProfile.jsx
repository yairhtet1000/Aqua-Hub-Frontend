...
const UserProfile = ({ own = false }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError("");
      try {
        const response = own ? await api.get("/user") : await api.get(`/users/${id}`);
        ...
        const postsRes = await api.get("/posts", { params: { user_id: userData.id } });
  ...
  if (loading) {
    return (
      <div className="mx-auto grid w-full max-w-[980px] gap-4 px-4 sm:px-6 lg:px-8" aria-label="Loading profile">
        <FeedSkeleton />
        <FeedSkeleton />
      </div>
    );
  ...
  const handle = profile.name?.replace(/\s/g, "_").toLowerCase() || "aquahub_member";

  return (
    <div className="mx-auto grid w-full max-w-[980px] gap-5 px-4 sm:px-6 lg:px-8">
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm shadow-slate-900/5 dark:border-slate-800 dark:bg-slate-950">
        ...
        <div className="grid gap-4 p-4 sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:items-end sm:p-5">
          ...
          <div className="min-w-0 sm:pb-1">
            <p className="truncate text-sm font-bold text-teal-700 dark:text-teal-300">@{handle}</p>
      ...
      <section className="grid grid-cols-2 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm shadow-slate-900/5 dark:border-slate-800 dark:bg-slate-950 sm:grid-cols-4">
        {[
          ...
          ["Followers", <Link key="followers" to="/following" className="hover:text-teal-700 dark:hover:text-teal-300">{followersCount}</Link>],
          ["Following", <Link key="following" to="/following" className="hover:text-teal-700 dark:hover:text-teal-300">{followingCount}</Link>],
          ...
          <div
            ...
            key={label}
          >
            <strong className="font-display text-2xl font-extrabold text-teal-800 dark:text-teal-300">{value}</strong>
      ...
      <section className="grid gap-4">
        <div className="flex items-end justify-between gap-4 px-1">
          <div className="min-w-0">
            <p className="text-xs font-bold uppercase tracking-[0.12em] text-teal-700 dark:text-teal-300">Activity</p>
            <h2 className="mt-1 break-words font-display text-2xl font-extrabold text-slate-950 dark:text-white">
        ...
        {userPosts.length === 0 ? (
          <PageEmptyState
            title={own ? "Your profile is ready for a first post" : "No posts yet"}
            description={own ? "Share a tank journal or ask the community for help." : "This aquarist has not shared a conversation yet."}
